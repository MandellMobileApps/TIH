...
-(void) startMgOperation
{
    ...
    self.urlRequest = [NSURLRequest requestWithURL:self.url cachePolicy:NSURLRequestReloadIgnoringLocalCacheData timeoutInterval:20];
...
//     [NSURL URLWithString:[NSString stringWithFormat:@"http://sampleAPI.com/api/v1/users?username=%@&password=%@", @"john89", @"123456"]];
...
-(void)connectionDidFinishLoading:(NSURLConnection *)connection {
 ...
 //   NSLog(@"%@: rawString: %@\n\n\n",[NSDate date],self.rawResponse);
    ...
    if ([self.webData length] == 0) {
        ...
        NSError *error = nil;
        self.json = [NSJSONSerialization JSONObjectWithData:self.webData options:NSJSONReadingAllowFragments error:&error];
